...
const parseSeminars = (html: string) => {
	...
	const pageDocumentElement = parser.parseFromString(
		html,
		"text/html",
	).documentElement;

	const table = pageDocumentElement.querySelector(
		"table.data1.zakladni_prihlasovani > tbody",
	);
...
const parseSeminar = (element: HTMLTableRowElement) => {
	...
	const registerA = element.querySelector(
		'td > font > a.okno[href^="./student"][target="_blank"]',
	);
	if (!(registerA instanceof HTMLAnchorElement))
		return new Error("No register a element");
...
const parseTeacher = (seminarDiv: Element) => {
	...
	if (!(teacherA instanceof HTMLAnchorElement))
		return new Error("No teacher a element");

	const name = teacherA.textContent;
	const link = teacherA.href;

	const ratingA = seminarDiv.querySelector(
		"span.nedurazne > a[href^='/auth/pruzkumy/odpovedi']",
	);
	const ratingLink =
		ratingA instanceof HTMLAnchorElement ? ratingA.href : undefined;
...
const parseSchedule = (scheduleString: string) => {
	const regexResults = /^.* (.+) (\d\d?:\d\d)–(\d\d?:\d\d) $/.exec(
		scheduleString,
	);
	...
	if (!day || !startTime || !endTime)
		return new Error("No day, startTime or endTime");
...
const getSubjectInfos = (subjectsInfoDiv: Element) => {
	const subjectsInfoA = Array.from(
		subjectsInfoDiv.querySelectorAll('a[href^="./student"]'),
	);

	const subjectsInfo: SubjectInfo[] = [];
	for (const subjects of subjectsInfoA) {
		if (!(subjects instanceof HTMLAnchorElement))
			return new Error("Not an anchor element");
