...
export const addPdfButtons = () => {
  const data = document.querySelectorAll('object[data^="/pdfjs/web/viewer.html"]');

  data.forEach((element) => {
    const previousElement = element.previousElementSibling;
    if (previousElement instanceof HTMLElement && previousElement.dataset.enhancedIs === "true")
      return;

    const data = element.attributes.getNamedItem("data")?.value;
    if (!data) return;

    const pdfLink = /^\/pdfjs\/web\/viewer\.html\?file=(.*.pdf)$/.exec(data)?.[1];
    if (!pdfLink) return;
